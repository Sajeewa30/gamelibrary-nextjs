...
const DiscoverPage = () => {
  ...
  const loadGames = async () => {
    ...
    try {
      const res = await fetch(
        `${API_BASE_URL}/public/igdb/games?year=${year}&page=1&pageSize=50`
      );
      if (!res.ok) {
        throw new Error("Failed to load games from IGDB.");
      }
  ...
  const openAddModal = (game: IgdbGame) => {
  ...
  const handleAddSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const releaseYear = selectedGame.releaseDate
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <h1 className="text-4xl font-semibold text-white/90">
              IGDB game list explorer
            </h1>
            <p className="mt-2 text-white/60">
              Pick a year, explore games, then add your favorites to the library.
            </p>
          </div>
          <Link
            href="/"
        ...
        <div className="rounded-3xl border border-white/10 bg-white/5 p-6 shadow-2xl shadow-black/30 backdrop-blur-2xl">
          {loading ? (
            ...
            <p className="text-center text-lg text-white/70">
              Choose a year to load games from IGDB.
            </p>
          ) : (
                  ...
                  >
                    ...
                    <div className="flex flex-1 flex-col gap-3 p-5">
                      ...
                      <p className="line-clamp-3 text-sm text-white/70">
                        {game.summary || "No summary available from IGDB."}
                      </p>
